...
app.post('/admin_login', (req,res)=>{
    if(req.body.email && req.body.pass){
        if(req.body.email=="test@test.com" && req.body.pass=="123"){
            // res.send("ADMIN PAGE");
...
//user result
app.post('/user_result', (req,res)=>{
    console.log(req.body.aadhaar);
    if(req.body.aadhaar){
        User_Appts.findOne({
            where: {aadhaar: req.body.aadhaar}
        }).then((user)=>{
            if(!user){
        ...
        res.render('error', {error: "Please enter all details!"});
    }
})
...
app.post('/update_result', (req,res)=>{
    if( req.body.results && req.body.aadhaar){
        User_Appts.findOne({
            where: {aadhaar : req.body.aadhaar}
        }).then((user)=>{
            let results = req.body.results;
            User_Appts.update({ result: results }, {
                where: {
                    aadhaar: req.body.aadhaar
                }
            });
           alert("Result Updated")
            return res.sendFile(__dirname + '/public/admin_page.html');
        }).catch((error)=>{
            console.log(error);
            return res.render('error', {error, text: "Some error occurred!."});
        })
    }
    else{
        res.render('error', {error: "Incomplete Details"});
    }
})
